...
export default function LeadMagnets() {
  ...
  const leadMagnets = [
    ...
    },
    {
      ...
      href: '/resources/macro-calculator',
    },
    {
      icon: 'podcasts',
      badge: 'Free Podcast',
      title: 'Hormone Optimization Podcast',
      description: 'Learn the Third Path approach to therapeutic hormone optimization. Real strategies for men 35+ looking to optimize performance safely.',
      href: 'https://open.spotify.com/episode/6uLxcZbAPoUZmLrpMIgRvx?si=CedGCMd9QHqJGoov8nSckQ&nd=1&dlsi=b300d84035654b72',
      external: true,
    },
  ]
            ...
            >
              <div className="absolute top-0 right-0 p-4 opacity-10">
                <span className="material-icons text-6xl text-white">
                  {magnet.icon === 'assignment_turned_in' ? 'medical_services' : magnet.icon === 'podcasts' ? 'podcasts' : 'fitness_center'}
                </span>
              </div>
              <div className="relative z-10">
                ...
                <button
                  onClick={() => openModal('lead-magnet', magnet.href, magnet.title)}
                ...
                >
                  {(magnet as any).external ? 'Listen Now' : 'Get Access'}
                  <span className="material-icons text-xl">{(magnet as any).external ? 'open_in_new' : 'arrow_forward'}</span>
                </button>
              </div>
            </div>
          ))}
